...
int check_col_up(int grid[4][4], int gap, int patern[16])
{
	int i;
	int max;
	int count;
...
int check_row_right(int grid[4][4], int gap, int patern[16])
{
	...
	if (gap % 4 == 3)
	{
		...
		/* compare if the result is as expected */
		if (patern[12 + gap / 4] != visible_towers)
			return (1);
	}
	return (0);
}

int check_col_down(int grid[4][4], int gap, int patern[16])
{
	int i;
	int max;
	int count;
	...
	if (gap / 4 == 3)
	{
		...
		/* compare if the result is as expected */
		if (patern[4 + gap % 4] != count)
			return (1);
	}
	return (0);
}

int check_row_left(int grid[4][4], int gap, int patern[16])
{
	int i;
	int max;
	int count;
	...
	if (gap % 4 == 3)
	{
		...
		/* compare if the result is as expected */
		if (patern[8 + gap / 4] != count)
			return (1);
	}
	return (0);
}

int check_cell(int grid[4][4], int gap, int patern[16])
{
	/* test for all possibilities if current case if good */
	if (check_row_left(grid, gap, patern) == 1)
		return (1);
	if (check_row_right(grid, gap, patern) == 1)
		return (1);
	if (check_col_down(grid, gap, patern) == 1)
		return (1);
	if (check_col_up(grid, gap, patern) == 1)
		return (1);
	return (0);
}
